/** Parula-inspired colormap (same table surfacefun-interactive uses). */
...
/** CSS gradient stops sampling the colormap, for the colorbar overlay. */
export function colormapGradient(direction: string): string {
  const stops: string[] = []
  const n = 32
  for (let i = 0; i < n; i++) {
    const t = i / (n - 1)
